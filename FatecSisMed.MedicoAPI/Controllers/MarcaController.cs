...
using FatecSisMed.MarcaAPI.Services.Interfaces;
...
namespace FatecSisMed.MedicoAPI.Controllers
{
    ...
    public class MarcaController : Controller
    {
        ...
        {
            var marcasDTO = await _marcaService.GetMarcaMedicos();
            if (marcasDTO is null)
        ...
        public async Task<ActionResult<MarcaDTO>> Get(int id)
        {
            ...
            if (marcaDTO == null) return NotFound("Marca não encontrada!");
            return Ok(marcaDTO);
        }
        ...
        {
            if (marcaDTO is null) return BadRequest("Dados inválidos!");
        ...
        {
            var marcaDTO = await _marcaService.GetById(id);
            if (marcaDTO is null) return NotFound("Marca não encontrada!");
